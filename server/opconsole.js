...
const ITEM_BY_WORD=[['брикет',40],['резак',41],['камер',42]];
...
class OpConsole {
  ...
  hb(b,replay){ const S=this.station; const was=JSON.stringify([S.bio,S.cam,S.grow,S.brik,S.cut]); Object.assign(S,{bio:b[0],cam:b[1],grow:b[2]===255?null:b[2],brik:b[3],cut:b[4],at:this.tNow}); const n=b[5]; const changes=[];
    const listed=new Set(); for(let i=0;i<n;i++){ const id=b[6+i*5], f=b[7+i*5], ch=b[8+i*5]/2.55, it=b[9+i*5], snr=b[10+i*5]-30; listed.add(id); const u=this.unit(id); const was={alive:u.alive,carrier:u.carrier,camera:u.camera,sonar:u.sonar,atAirlock:u.atAirlock,hb:u.hbAt!==undefined};
      Object.assign(u,{alive:!!(f&1),carrier:!!(f&2),camera:!!(f&4),sonar:!!(f&8),streaming:!!(f&16),atAirlock:!!(f&32),charge:ch,snr,hbAt:this.tNow,items:[...Array(it&3).fill(40),...(it&4?[41]:[])]});
  ...
  parse(line){
    ...
    if(/^станция/i.test(m[1])){
      ...
      return {error:'станция: статус | вырастить [камера] [лидар] | пульс каждые N | кадр [8|16|32|64] [дельта] | автосъёмка каждые N [32] [дельта] | выкл'}; }
    ...
    if(/^(полож|сдать|сдай|брос)/.test(v)){ const it=itemOf(v); if(!it) return {error:'положить: брикет / резак / камеру'}; if(/^сда|на склад/.test(v)) return {bytes:[21,it,unit,0], label:`М${unit} сдать на склад ${ITEMS[it]}`}; if(/на грунт|^брос/.test(v)) return {bytes:[22,it,unit,0], label:`М${unit} сбросить ${ITEMS[it]}`}; const o=/\sв\s/.test(v)?this.findKnown(v.replace(/^.*?\sв\s/,'')):null; if(!o) return {error:'положить X в <контейнер> | на грунт | сдать X (склад)'}; return {bytes:[22,it,unit,o.id], label:`М${unit} положить ${ITEMS[it]} в ${o.name}`}; }
