...
| `crates_sheet` | — (был 2, ориентир «штабель») | 2,4 | два яруса, крышки открыты; не используется с 20.09.2026: штабель стал указателем, ящики 13/14 рисуются `crate_sheet` |
| `crate_sheet` | 13, 14 | 0,9 | одиночный ящик с маркировкой и пломбой (20.09.2026) |
...
| `cabinet_sheet` | 18, 35, 36 | 1,8 (36 — 0,9) | шкаф с открытой дверцей, усилитель, индикатор; тот же лист — стационарный ретранслятор (35) и, пока нет своего, переносной (36) |
...
- Слипшиеся виды (ступни в шаге, широкие опорные плиты турели) — сборщик делит сам, пока видов меньше пяти: ориентир ширины — медиана, но не шире пятой части листа; если делит неверно, проще перегенерировать, чем править.
...
- Платформа: спрайт, корпус для лидара и столкновений и знак на карте — один эллипс `STATION` в кодовой книге (16×9 м, центр 6,0). Обломки — коллайдер типа 23 в кодовой книге (круг r 5, h 4) при спрайте 12 м шириной; люк 24 стоит на кромке коллайдера (act1: 5,3 м от центра, курсом наружу), гарь 25 — круг 7 м под обломком, край виден.
- Кредиты SpriteCook на 20.09.2026 — 239 (`get_credit_balance`).
...
2. Турель — второе состояние «повреждена» (головка сорвана, лампа разбита) и «выключена» (лампа тёмная) — когда будет механизм состояний из п. 1.
3. Пирамидка тропы — контрастнее (см. §7).
3а. Переносной ретранслятор (тип 36, `SPRITES[36]`): ящик-кейс с антенной-штырём, 0,9 м; сейчас — уменьшенный `cabinet_sheet`.
4. Крупные обломки у подножия обрыва (2 листа), чтобы осыпь читалась с 25 м.
5. **Ящик, второе состояние** (13 вскрыт / 14): `crate_sheet` есть (20.09.2026), нужен лист с открытой крышкой — когда появятся вторые состояния (п. 1). Надпись 15 — плоский рисунок на грунте или на борту ящика, отдельный лист не нужен.
6. **Спрайт каждому объекту** (design.md §11г): без листа остаются 15 надпись, 26 темнота (текст у входа — возможно, станет текстом указателя). Редактор помечает такие объекты пустым кружком и строкой в правилах карты.
