...
//       log(rec) — лог мира (tech.md §12): команды, доставленные пакеты, потери, заметки мира, раз в секунду — правда о положениях. Хост пишет, куда хочет.
  ...
  function tickOnce(){
    ...
    if(dbg && opts.log){ const sec=Math.floor(links[0].link.t+1e-6); if(sec!==lastLogSec){ lastLogSec=sec; log({k:'phys', units:dbg.units, pack:dbg.pack, turrets:dbg.turrets, relays:dbg.relays, ground:dbg.ground, stations:dbg.stations, snr:links.map(L=>Object.fromEntries(Object.keys(L.link.phys.units).map(id=>[id,+L.link.snrDb(+id).toFixed(1)]))) }); } }   // правда о мире целиком — по ней спектатор проигрывает сеанс
    }
