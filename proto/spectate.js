...
// на сервере ?room=КОД — тот же лог живьём по WebSocket, без перемотки (tech.md §12); постфактум с ползунком — ?log=/log/КОД.log (лог планеты с сервера). Мир в логе
...
function noCarrier(c,X,Y,col){ c.save(); c.strokeStyle=col; c.lineWidth=1.5; c.beginPath(); c.arc(X,Y,4,0,7); c.moveTo(X-3,Y+3); c.lineTo(X+3,Y-3); c.stroke(); c.restore(); }   // нет несущей (центр знака X, Y): после имени тела, как † у мёртвого; станция тело не слышит
...
function parseLine(line){ if(!line.trim()) return; let r; try{ r=JSON.parse(line); }catch(e){ return; } const t=+r.t||0;
  if(r.k==='phys'){ frames.push({t, units:r.units||[], pack:r.pack||[], turrets:r.turrets||[], relays:r.relays||[], ground:r.ground||[], stations:r.stations||null}); if(live.on){ live.lastPhys=performance.now(); if(frames.length>LIVE_KEEP) frames.splice(0,frames.length-LIVE_KEEP); } return; }
...
function frameAt(t){ if(!frames.length) return null; let lo=0, hi=frames.length-1; while(lo<hi){ const m=(lo+hi+1)>>1; if(frames[m].t<=t) lo=m; else hi=m-1; }
  ...
  return { t, units:a.units.map(u=>{ const v=b.units.find(v=>v.id===u.id); return v&&u.alive?mix(u,v):u; }), pack:a.pack.map(p=>{ const q=b.pack[p.i]; return q&&p.act!=='dead'?mix(p,q):p; }), turrets:a.turrets, relays:a.relays, ground:a.ground, stations:a.stations }; }
...
function draw(){ ctx.fillStyle='#000'; ctx.fillRect(0,0,W,H);
  ...
  for(const u of f.units){ const X=S(u.x),Y=Sy(u.y), col=stCol(u.st||0);   // мёртвое — тем же цветом команды: крест вместо квадрата, серое терялось на карте высот
    ...
    const lab=`М${u.id}${u.alive?(u.reflex===5?' бой':u.reflex===6?' бегство':u.stealth?' тихо':''):' †'}`; if(layers.labels){ ctx.fillStyle=col; ctx.fillText(lab,X+7,Y-8); }
    if(u.alive&&u.car===false) noCarrier(ctx,layers.labels?X+7+ctx.measureText(lab).width+7:X+10,Y-8,col); }   // статус — после имени: † мёртв, ⊘ нет несущей
  // особи
...
function findSel(f,sel){ if(sel.kind==='unit') return f.units.find(u=>u.id===sel.id); if(sel.kind==='pack') return f.pack[sel.i]; if(sel.kind==='turret') return f.turrets[sel.k]; if(sel.kind==='relay') return (f.relays||[]).find(R=>R.id===sel.id); if(sel.kind==='ground') return f.ground.find(g=>g.id===sel.id); if(sel.kind==='station'){ const B=bases().find(B=>B.k===sel.k); return B&&{...B, S:(f.stations||[]).find(S=>S.k===sel.k)||null}; } return null; }
function hit(px,py){ const f=frameAt(cur); if(!f) return null; let best=null, bd=14;
  ...
  f.units.forEach(u=>test(u,{kind:'unit',id:u.id})); f.pack.forEach(p=>test(p,{kind:'pack',i:p.i})); f.turrets.forEach((T,k)=>T&&test(T,{kind:'turret',k})); (f.relays||[]).forEach(R=>test(R,{kind:'relay',id:R.id})); f.ground.forEach(g=>test(g,{kind:'ground',id:g.id})); if(!best) for(const B of bases()) if(Math.hypot((px-S(B.x))/Math.max(STATION.rx*sc,8),(py-Sy(B.y))/Math.max(STATION.ry*sc,8))<1) best={kind:'station',k:B.k}; return best; }   // станция — по корпусу, если рядом нет ничего мельче

// ---------- характеристики ----------
function kv(rows){ return '<div class="kv">'+rows.filter(r=>r[1]!==undefined&&r[1]!==null&&r[1]!=='').map(([k,v])=>`<label>${k}</label><span>${v}</span>`).join('')+'</div>'; }
function renderInfo(){ const sel=pinned||hover, P=$('#info'); if(!sel){ P.innerHTML='<span class="dim">Наведи на тело, особь, станцию, турель, ретранслятор или свёрток. Клик — закрепить.</span>'; return; }
  ...
  else if(sel.kind==='station'){ const S=o.S, own=f.units.filter(u=>(u.st||0)===o.k), T=f.turrets.filter(T=>T&&T.st===o.k), it=S&&S.store||{};   // станция: запасы и выращивание — из записи phys (логи до 26.09.2026 их не несут)
    P.innerHTML=`<h3>ARK-04${1+o.k}${pin}</h3>`+kv([['операторы',opsAt(o.k,cur).join(', ')||'—'],...(teams.length?[['команда',1+teamOf(o.k)]]:[]),['миссионеры',`${own.filter(u=>u.alive).length} жив. из ${own.length}`+(own.length?' — '+own.map(u=>'М'+u.id+(u.alive?'':' †')).join(', '):'')],
      ...(S?[['выращивание',S.grow!=null?`тело через ${Math.floor(S.grow/60)}:${String(S.grow%60).padStart(2,'0')}${S.growCam?' (с камерой)':''}`:'не идёт'],['биоматериал',S.bio],['склад',`камер ${S.cam}, брикетов ${it[40]||0}, резаков ${it[41]||0}${it[44]?', планшет':''}`],['питание',S.power],['канал',S.freq],['задача ПС-7',S.task?(S.closed?'серия закрыта: тел и биоматериала нет':'открыта'):'закрыта']]:[['запасы','в этой записи нет — лог записан до 26.09.2026']]),
      ['турели',T.map(T=>`${T.id}: ${T.broken?'повреждена':!T.powered?'без питания':T.on?'включена':'выключена'}, патронов ${T.ammo}`).join('; ')||'—']]); }
