...
function draw(){ ctx.fillStyle='#000'; ctx.fillRect(0,0,W,H);
  ...
  for(const [k,T] of f.turrets.entries()){ if(!T) continue; const X=S(T.x),Y=Sy(T.y);
    if(layers.sectors){ ctx.fillStyle=T.on?'rgba(255,220,120,0.07)':'rgba(120,120,120,0.05)'; ctx.strokeStyle=T.on?'rgba(255,220,120,0.35)':'rgba(120,120,120,0.3)'; ctx.beginPath(); ctx.moveTo(X,Y); ctx.arc(X,Y,T.range*sc,T.ang-T.fov/2,T.ang+T.fov/2); ctx.closePath(); ctx.fill(); ctx.stroke(); }
    ctx.fillStyle=T.on?'#ffdc78':'#777'; ctx.fillRect(X-3,Y-3,7,7);
    ...
    if(layers.labels&&sc>=1) { ctx.fillStyle='#ffdc78'; ctx.fillText(T.rel>0?`турель · перезарядка ${T.rel.toFixed(0)}`:'турель',X+7,Y+8); } }
  // свёртки
...
function renderInfo(){ const sel=pinned||hover, P=$('#info'); if(!sel){ P.innerHTML='<span class="dim">Наведи на тело, особь, турель или свёрток. Клик — закрепить.</span>'; return; }
  ...
  else if(sel.kind==='turret') P.innerHTML=`<h3>турель ARK-04${1+sel.k}${pin}</h3>`+kv([['положение',`${o.x}, ${o.y}`],['лампа',o.on?'горит':'не горит'],['сектор',`${(o.fov*180/Math.PI).toFixed(0)}° вокруг ${(o.ang*180/Math.PI).toFixed(0)}°`],['дальность',o.range+' м'],['цель',o.tgt?(o.tgt.p!==undefined?'О'+(o.tgt.p+1):'М'+o.tgt.u):'—'],['прицел',o.aim+' с'],['перезарядка',o.rel>0?o.rel+' с':'готова']]);
