...
// нет несущей: перечёркнутый кружок слева над меткой тела — тот же знак, что в спектаторе; по пульсу станции (флаг «несущая»)
function noCarrier(c,X,Y){ c.save(); c.strokeStyle='#d9534f'; c.lineWidth=1.5; c.beginPath(); c.arc(X-9,Y-9,4,0,7); c.moveTo(X-12,Y-6); c.lineTo(X-6,Y-12); c.stroke(); c.restore(); }
...
function drawMap(){ const cv=$('#map'); fitCanvas(cv,true); const ctx=cv.getContext('2d'), W=cv.width, H=cv.height; ctx.fillStyle='#000'; ctx.fillRect(0,0,W,H);
  ...
  ctx.globalAlpha=1;
  for(const u of units.values()){ const p=pos(u.id); const col=UCOL[(u.id-1)%UCOL.length]; ctx.fillStyle=u.alive?col:'#666'; ctx.beginPath(); ctx.arc(sx(p.x),sy(p.y),u.id===active?5:3.5,0,7); ctx.fill(); if(u.alive&&u.hbAt>-1e8&&!u.carrier) noCarrier(ctx,sx(p.x),sy(p.y)); ctx.fillStyle=col; ctx.fillText(`М${u.id}${u.alive?'':' †'}`,sx(p.x)+7,sy(p.y)-6); if(u.tlm&&tNow-u.tlmAt>10){ ctx.fillStyle='#888'; ctx.fillText(`${(tNow-u.tlmAt).toFixed(0)} с назад`,sx(p.x)+7,sy(p.y)+6); } }
