...
function decodeTlm(pkt){ const b=pkt.bytes, u=U(pkt.unit); if(boot.onTlm) boot.onTlm(pkt);
  u.tlm={pulse:b[0],electro:b[1],glucose:b[2],toxin:b[3],skin:b[4],bone:b[5],psyche:b[6],danger:b[7],cons:b[8]/50,charge:b[9]/2.55,gen:b[10]/50,x:(((b[11]<<8)|b[12])-32768)/10,y:(((b[13]<<8)|b[14])-32768)/10,mode:b[15]&7,stealth:!!(b[15]&8),stance:(b[15]>>4)&3,reflex:b[15]&64?5:b[15]&128?6:0};   // байт 15: режим | скрытность·8 | стойка·16 | рефлекс 64/128
  { const r=u.req; if(r){ for(const k of ['mode','stealth','stance']) if(r[k]!==undefined && r[k]===u.tlm[k]) delete r[k]; if(tNow-r.at>90) u.req=null; } }   // запрошенное подтвердилось — контур снимается
...
function decodeEvt(pkt){ const b=pkt.bytes, code=b[0], arg=b[1], un=pkt.unit?`М${pkt.unit} `:''; const txt=EVENTS[code]||('событие '+code);
  if(code===7) log(`${un}${txt}: ${MODES[arg]}`,'evt'); else if(code===32) log(`${un}${txt}: ${arg?'включена':'выключена'}`,'evt'); else if(code===33) log(`${un}${txt}: ${STANCES[arg]}`,'evt'); else if(code===13) log(`${un}${txt} М${arg}`,'evt'); else if(code===16) log(`${un}${txt} (id ${arg}); требуется новое описание`,'err'); else if(code===19||code===20) log(`${un}${txt}: ${ITEMS[arg]||arg}`,'evt'); else if(code===28) log(`${un}${txt} (${arg*10} м от ближайшего узла${station.returnR?', предел '+station.returnR+' м':''})`,'err'); else log(`${un}${txt}`,'evt');
