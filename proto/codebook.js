...
// Исключение — корпоративная номенклатура (corp): её тип идёт в описании номером, и консоль показывает команды по кодовой книге (objCmds).
// Консоль отсюда берёт системные коды (события, режимы, предметы), интерфейс корпоративных типов и кодек текста.
//   states  — что видно вблизи в каждом состоянии (ответ на «изучить»)
//   actions — что можно сделать из состояния: {from, to, verb, item?, needs?, req?, fail?, cmd?, afterExam?}
...
//     cmd   — имя команды для консоли (только у корпоративных типов, см. corp); afterExam — кнопка появляется после осмотра
//   corp    — корпоративная номенклатура (части базы, аппаратура, принесённое с Земли): станция знает тип заранее — в описании идёт его номер,
//             консоль показывает именные команды из кодовой книги (objCmds). Остальное — природное и неизвестное: тип в описании 0,
//             одна кнопка «взаимодействовать», результат — по состоянию (design.md §11г)
//   cmds    — именные команды, которые выполняет мир (турель, ретранслятор): [{from, label, needs?}] — по состоянию; «действие» (команда 8) одна на всех
//   accepts — что объект принимает как заправку (команда «положить» с этим предметом): турель — патроны; container — контейнер для любых мелких (ITEM_PROPS.small)
const CODEBOOK = {
  ...
  10: { key:'hatch',        name:'люк',                 states:['Закрыт. Маховик заклинил.','Маховик срезан. За люком — переходной отсек, пустой. На стене мелом: «7».'], actions:[{from:0,to:1,verb:'срезал маховик',needs:41,fail:'Маховик не поддаётся. Нужен инструмент.',cmd:'срезать маховик'}], corp:1 },
  11: { key:'floodlight',   name:'прожектор',           states:['Не горит. Плафон разбит изнутри.'], corp:1 },
  12: { key:'footprints',   name:'следы',               states:['Отпечатки в грунте. Босые. Разного размера. Ведут в обе стороны.'] },
  13: { key:'crate',        name:'ящик',                states:['Запечатан. Пломба цела.','Вскрыт.'], actions:[{from:0,to:1,verb:'вскрыл',cmd:'вскрыть'}], container:1, corp:1 },
  14: { key:'crate_open',   name:'вскрытый ящик',       states:['На крышке процарапано слово.','Процарапано: «НЕ ЖДИТЕ».'], actions:[{from:0,to:1,verb:'прочитал надпись',cmd:'прочитать надпись'}], container:0, corp:1 },
  ...
  16: { key:'cable',        name:'кабель',              states:['Тянется к мачте. Перебит.'], corp:1 },
  17: { key:'mast',         name:'мачта',               states:['Держится на двух растяжках из четырёх.'], corp:1 },
  18: { key:'cabinet',      name:'шкаф аппаратуры',     states:['Дверца открыта. Внутри пусто: блок снят.'], corp:1 },
  19: { key:'cable_cut',    name:'оборванный кабель',   states:['Срез ровный. Инструментом.','Срощен. Изоляция — обмотка из комбинезона.'], actions:[{from:0,to:1,verb:'срастил кабель',needs:41,fail:'Жилы нужно зачистить. Нужен инструмент.',cmd:'срастить'}], corp:1 },
  ...
  32: { key:'tablet',       name:'планшет',             states:['Корпоративный планшет, экран треснут. Индикатор заряда мигает.','Последняя запись, 39 лет назад: «Протокол требует возврата. Не вернусь. Станция вырастит следующего, и следующего, и следующего — пока есть биоматериал. Я закопал шестерых. Больше не буду. Если это читает оператор: отключите протокол ПС-7. Я останусь здесь. М-07.» Ниже — отметки дней. Много. Последняя — сегодня.','Планшета здесь больше нет.'], actions:[{from:0,to:1,verb:'прочитал запись',special:'finale',cmd:'прочитать запись',afterExam:1}], corp:1, pickup:44, gone:2 },   // pickup — «взять»: объект становится предметом (носимое), состояние gone — объекта в мире больше нет
  ...
  33: { key:'bundle',       name:'свёрток',             states:['Оставлено на грунте.'], container:0, corp:1 },
  34: { key:'turret',       name:'турель',              states:['Включена. Лампа горит, ствол ведёт по сектору.','Выключена. Лампа не горит.','Повреждена. Головка сорвана с оси, лампа разбита.'], corp:1, cmds:[{from:0,label:'выключить'},{from:1,label:'включить'}], foe:[{from:0,label:'резать',needs:41},{from:1,label:'резать',needs:41}], accepts:[43], fill:'заправить' },   // действия — в мире: своя вкл/выкл (cmds), чужая — резать (foe); патроны — «положить» (accepts)
  ...
  35: { key:'relay',        name:'ретранслятор',        states:['Выключен.','Включён.','Питания нет.'], relay:{range:2000, gain:6}, corp:1, cmds:[{from:0,label:'включить на канал станции'},{from:1,label:'переключить'}] },
  36: { key:'relay_mobile', name:'переносной ретранслятор', states:['Выключен.','Включён.','Питания нет.'], relay:{range:400, gain:0}, corp:1, cmds:[{from:0,label:'включить на канал станции'},{from:1,label:'переключить'}] },
  252:{ key:'missionary',   name:'миссионер',           states:['Наш. Идёт.'] },
};

const ITEMS = { 40:'пищевой брикет', 41:'резак', 42:'камера', 43:'патроны', 44:'планшет' };   // патроны — пачка на 8 выстрелов, кладётся в турель; планшет — берётся у объекта 32 (pickup), сдаётся на склад
// Свойства предметов: small — мелкий, кладётся в любой контейнер. Носимое, но не мелкое (тело миссионера — в биореактор) — потом: carry без small
const ITEM_PROPS = { 40:{small:1}, 41:{small:1}, 42:{small:1}, 43:{small:1}, 44:{small:1} };
// Интерфейс объекта для консоли — по типу и состоянию из описания, без запроса к миру (design.md §11г). Тип 0 (не корпоративный) — одна кнопка
// «взаимодействовать». ctx.own — своя ли турель (по пульсу станции). Возвращает [{kind:'act'|'fill', label, needs?, afterExam?, item?}]
function objCmds(type,state,ctx={}){ const cb=CODEBOOK[type]; if(!cb||!cb.corp) return [{kind:'act',label:'взаимодействовать'}];
  const out=[]; const list=(cb.foe&&ctx.own===false)?cb.foe:(cb.cmds||[]);
  for(const c of list) if(c.from===undefined||c.from===state) out.push({kind:'act',label:c.label,needs:c.needs,afterExam:c.afterExam});
  for(const a of cb.actions||[]) if(a.from===state&&a.cmd) out.push({kind:'act',label:a.cmd,needs:a.needs,afterExam:a.afterExam});
  for(const it of cb.accepts||[]) if(ctx.own!==false) out.push({kind:'fill',label:`${cb.fill||'заправить'}: ${ITEMS[it]}`,item:it});
  if(cb.pickup&&state!==cb.gone) out.push({kind:'take',label:'взять',item:cb.pickup});   // объект-предмет: взять с собой (команда 23 с id объекта)
  return out; }
