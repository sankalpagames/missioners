...
const CAM = (()=>{
  ...
  function renderRaw(u,size,objs,opt={}){ const W=size, H=opt.H||size, fov=opt.fov||FOV;
    ...
    for(let y=0;y<H;y++)for(let x=0;x<W;x++){ const ax=hd+Math.atan((x-W/2+0.5)/f); const dzs=-(y-horizon)/f; const n3=Math.hypot(1,dzs);
    ...
    for(let i=0;i<W*H;i++){ if(depth[i]===Infinity||!floor[i]) continue; const px=gx[i], py=gy[i]; let dark=0, add=0;
    ...
    for(const o of solid.filter(o=>Math.abs(o.rb)<fovR*1.4).sort((a,b)=>a.r-b.r)){ const sp=SPRITES[o.type]; const Ho=Hof(o); const rz=o.r*Math.cos(o.rb); if(rz<0.3) continue;
      ...
      const sx=W/2+f*Math.tan(o.rb); const x0=Math.round(sx-wpx/2), x1=Math.round(sx+wpx/2), y0=Math.round(yb-hpx), y1=Math.round(yb);
      if(x1-x0<1&&y1-y0<1){ const xi=Math.round(sx), yi=Math.round(yb-0.5); if(xi>=0&&xi<W&&yi>=0&&yi<H&&o.r<depth[yi*W+xi]) img[yi*W+xi]=clamp(fog(70,o.r)*lit); continue; }
      for(let yy=Math.max(0,y0);yy<Math.min(H,y1);yy++) for(let xx=Math.max(0,x0);xx<Math.min(W,x1);xx++){ const i=yy*W+xx; if(o.r>=depth[i]) continue;
  ...
  return { load, build, render, renderRaw, get ready(){ return !!atlas; } };
})();
