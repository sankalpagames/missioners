...
const wsrc=[require('./map.js').mapSrc(), ...['codebook.js','terrain.js','camera.js','world.js'].map(f=>fs.readFileSync(P+f,'utf8'))].join('\n')+'\nreturn {sonar, TER, units};';
...
const C=new Function(csrc+'\nreturn {HCELL, hmap, hwall, hmapAdd, hmapSmooth, hmapContours, snapWalls};')();
...
for(let d=0;d<=L;d+=8){ const u={id:1,x:A.x+(B.x-A.x)*d/L,y:A.y+(B.y-A.y)*d/L,sensors:{sonar:true},sonarTilt:0,items:[]};
  for(const k of TILTS){ u.sonarTilt=k; msgs.length=0; W.sonar(u); const b=msgs[0].payload; nsnap++;
    ...
    const rays=[...b.slice(15,79)], mask=[...b.slice(7,15)]; C.hmapAdd(x,y,tilt,zs,rays,mask); if(!OLD) C.snapWalls({x,y,b:rays,m:mask,k:tilt}); } }
console.log(`снимков ${nsnap}, ячеек ${C.hmap.size}, ячеек стен ${C.hwall.size}`);
...
console.log(`ошибка высоты ячейки: медиана ${es[es.length>>1].toFixed(2)} м, p90 ${es[(es.length*.9)|0].toFixed(2)}, максимум ${es[es.length-1].toFixed(2)} (в максимуме — подошва стены: отсчёт с дна в ячейке 2 м, центр которой уже на скале; сплошные отсчёты в карту не идут)`);
