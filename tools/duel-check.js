...
if(tab&&/прибыл/.test(tab)){ run(1,'М: описание',/планшет/,60,'планшет в описании'); run(1,'М: изучить планшет',/осмотр/,60,'осмотр планшета'); }
...
console.log(`\n— сводка: часы мира ${mmss(T())}; средний ретранслятор: ${R(2).on?'включён':'выключен'}, канал ${R(2).freq}; входной: ${R(5).on?'включён':'выключен'}, канал ${R(5).freq}; `+
  d.units.map(u=>`М${u.id} ${d.stations[u.st].name} (${u.x.toFixed(0)},${u.y.toFixed(0)})${u.alive?'':' мёртв'}`).join(', ')+
