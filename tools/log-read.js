...
function line(r){ const n=r; switch(r.k){
  ...
  case 'note': switch(r.kind){
    case 'cmd': { let a=''; if(r.cmd==='режим') a=MODES[r.arg]||r.arg; else if(r.cmd==='идти'||r.cmd==='смотреть'){ const b=r.bytes; a=`(${(((b[0]<<8)|b[1])-32768)/10}, ${(((b[2]<<8)|b[3])-32768)/10})`; } else if(r.arg) a=String(r.arg); return `ARK-04${1+r.st} → ${r.unit?'М'+r.unit+' ':''}${r.cmd}${a?' '+a:''}`; }
    case 'cry': return `   #${r.who} кричит «${r.word}» из (${r.x}, ${r.y}); слышат ${r.heard.length?r.heard.map(i=>'#'+i).join(' '):'— никто'}`;
    case 'pack': if(r.sense) return `   #${r.who} ${r.sense==='see'?'увидела':'услышала'} М${r.unit} в ${r.d} м (${MODES[r.mode]||r.mode}${r.light?', фонарь':''}), nerve ${r.nerve}`;
      if(r.bite) return `   #${r.who} укусила М${r.bite}: кожа ${r.skin}, кости ${r.bone}${r.mode===5?'':' — тело не в режиме «бой»'}`;
