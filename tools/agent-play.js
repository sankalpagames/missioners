...
//   node tools/agent-play.js [ускорение] [voice=N] [map=ID] — по умолчанию ×4, радиус голоса ∞, карта act1 (proto/maps/ID.js)
...
st.log({k:'start', host:'play', n:st.NST, map:st.meta, wall:new Date().toISOString()}); st.handle({t:'speed',v:speed});
...
rl.on('line',line=>{ const s=line.trim(); if(!s) return; let m;
  ...
  else if(s==='где'){ const d=st.snapshot(); console.log('       [отладка, правда о мире] '+(d.turrets||[]).map(T=>`турель ${T.id} ${T.broken?'повреждена':!T.on?'выкл':T.tgt?'ведёт '+(T.tgt.p!==undefined?'О'+(T.tgt.p+1):'М'+T.tgt.u)+' '+T.aimT.toFixed(1)+' с':T.reloadT>0?'перезарядка':'ждёт'} · ${T.ammo} | `).join('')+d.pack.map(p=>`О${p.i+1}(${p.x.toFixed(0)},${p.y.toFixed(0)}) ${p.act}`).join(' ')+' | '+d.units.map(v=>`М${v.id}(${v.x.toFixed(0)},${v.y.toFixed(0)})${v.alive?'':' мёртв'}`).join(' ')); }
